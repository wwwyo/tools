...
```mermaid
flowchart TB
  input[入力テキスト] --> dict["辞書マッチ<br/>記号・書式層 + フレーズ層<br/>94 エントリ"]
...
1. **記号・書式層**: em ダッシュ、`**太字ラベル:**` 箇条書き、行頭絵文字、`§ ¶ •` など、LLM 出力の書式の残骸。人間はほぼ書かない形なので高 weight で決め手にする
2. **フレーズ辞書層**: ハイプ・誇張 / 定型評価語 / テンプレ導入・結び / 事なかれ・両論併記 / 冗長構文 / 行頭接続詞 / 急な英語の 7 カテゴリ。表層の部分一致で、意味的な曖昧マッチはしない
...
- **急な英語だけは検知語を列挙せず、置かれ方で拾ってから allowlist で引く**。AI が好む英単語を数え上げても網羅できないので、日本語に前後を挟まれた小文字ラテン語 1 語という形だけで拾い、技術・開発の文脈で普段そのまま打つ語を allowlist で落とす。weight は最弱クラスに置き、1 語では効かず連発ではじめて効く（allowlist の判断基準は同ディレクトリの AGENTS.md）
...
├── AGENTS.md       このツールを触るときの編集ルール
└── CLAUDE.md       AGENTS.md を読ませるだけ
```
